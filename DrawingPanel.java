 ...
 *IMPORTANT* Note to students: DO NOT modify this file!!!
 
  Your own .java files can create a DrawingPanel object by doing:
     
     DrawingPanel panel = new DrawingPanel();
     
  and then you can use it by calling its instance methods, like 
    panel.getGraphics(),
    panel.clear() 
 
  Besides what's described in the textbook Chapter 3G, Dr. Stonedahl 
  added a few nifty methods for doing stuff with the Mouse & Keyboard,
  so you can make your programs more interactive. These are described 
  briefly below, and he will provide some example code for you 
  when the time seems right. 
*/


// Specifically, Dr. Stonedahl added these methods:
//
//    public int getMouseX() // getthe current mouse location
//    public int getMouseY() 
//
//    public boolean checkMouseClicked() // was the mouse clicked since last time this method was called?
//    public int getMouseClickX() 
//    public int getMouseClickY() 

//    public boolean checkKeyboardInput() // has a key been pressed?  true/false
//    public String getKeyboardInput()  // what key was it?  "a" or "\n" or "Left" or ...?
//     
//    public void clearKeyboardInputQueue()  //  you can also clear the queue of pressed keys, 
//                                           // if there's backlog of key strokes you want to ignore.
//                                           // (you probably won't need to use this method...)
// 
//    public boolean isKeyDown(String key)  // check if a given key ("Left", "Right", "a", etc) is currently pressed down
//    public String getAllKeysDown()        // returns a string containing all the keys current down (separated by '~')  
//
//    public boolean isVisible() // tells you if the window is still open (true), or if it's been closed (false)
//
//    public Color getColorAtPixel(int x, int y) // what color is the pixel at these coordinates...
//
//    public void saveImageWithoutErrors(String imageFileName) // save what's currently displayed as an image file
//    public void loadImageAndResizeWindow(String imageFileName)  // loads an image from a file and displays it
//                           									  // (resizing the panel to fit it nicely) 
// 
...
@authors Marty Stepp (with minor modifications by Forrest Stonedahl)
...
public final class DrawingPanel extends FileFilter
    implements ActionListener, MouseMotionListener, MouseListener, Runnable, WindowListener, KeyListener {
    ...
    */
    ...
    private Queue<KeyEvent> keyInputQueue = new ConcurrentLinkedQueue<KeyEvent>();
    private Set<String> keysCurrentlyDown = new TreeSet<String>();
    ...
    public void keyPressed(KeyEvent e) {
		...
		keysCurrentlyDown.add(keyEventToText(e));
    }
    ...
    // ignores this key event...    
    public void keyReleased(KeyEvent e) { 
		keysCurrentlyDown.remove(keyEventToText(e));
    }
    
    
    /**
     * @return the current x-coordinate of the mouse in the drawing panel
     */
    public int getMouseX() {
    	return (int) this.lastMousePosition.getX();    	
    }
    /**
     * @return the current y-coordinate of the mouse in the drawing panel
     */
    public int getMouseY() {
    	return (int) this.lastMousePosition.getY();    	
    }
    /**
     * @return *true* if the mouse was clicked since the last time this method was called, *false* otherwise.
     */
    public boolean checkMouseClicked() {
        boolean answer = this.wasMouseClicked;
    	this.wasMouseClicked = false;
    	return answer;
    }
    /**
     * @return the x-coordinate of the last mouse click in the drawing panel
     */
    public int getMouseClickX() {
    	return (int) this.lastMouseClick.getX();    	
    }
    /**
     * @return the y-coordinate of the last mouse click in the drawing panel
     */
    public int getMouseClickY() {
    	return (int) this.lastMouseClick.getY();    	
    }
    
    /** Has the user pressed a key that hasn't been handled yet
     * by your program?
     */
    public boolean checkKeyboardInput() {
    	return !keyInputQueue.isEmpty();
    }
    /** This method returns a String representation of the key
     * that the user pressed.  For letters, you'd get Strings
     * of length one, like "A" or "q".
     * For SPACE BAR, you get " ", and for ENTER you get "\n".
     *   
     * For action keys, you might get longer strings like 
     * "Down","Left","Right", "Up", or "Caps Lock".
     */
    public String getKeyboardInput() {
    	if (!keyInputQueue.isEmpty()) {
    		return keyEventToText(keyInputQueue.poll());
    	}
    	return "";
    }
    /** This method clears out all the recorded keystrokes...
     * may be useful in case the user works up a backlog 
     * (especially by holding down keys...) 
     */
    public void clearKeyboardInputQueue() {
    	keyInputQueue.clear();
    }
    
    private String keyEventToText(KeyEvent ke) {
		if(ke.getKeyChar() != KeyEvent.CHAR_UNDEFINED) {
			return "" + ke.getKeyChar();
		}
		else {
			return KeyEvent.getKeyText(ke.getKeyCode());
		}    	
    }

    /** returns whether the specified key is currently being pressed down by the user */
    public boolean isKeyDown(String key) {
    	return keysCurrentlyDown.contains(key);
    }
    
    /** returns a String containing a list of ALL the keys that are currently being pressed.*/ 
    public String getAllKeysDown() {
    	return String.join("~",keysCurrentlyDown);
    }
    
    /** returns true if the window is visible, and false if it has been closed */
    public boolean isVisible() {
    	return frame.isVisible();
    }

    /** returns the color of the pixel at the point (x, y)? */
    public Color getColorAtPixel(int x, int y) {
    	BufferedImage canvasImage = getImage();
    	return new Color(canvasImage.getRGB(x, y));
    }
    
    /** Loads and displays the specified image file and 
     * also resizes the panel window to match the loaded image's size. */
    public void loadImageAndResizeWindow(String imageFileName) {
...
//			this.imagePanel.setImage(img);
			...
			System.err.println("ERROR loading image named: " + imageFileName);
			System.err.println("Full path: " + inputFile.getAbsolutePath());			
			System.err.println("Error message: " + ex);
    		System.err.println("Because of error, exiting program...");
			System.exit(1);
		}
    }
    /**
     * Saves the image displayed on this DrawingPanel to a file.
     * If any errors occur while saving, this method will print
     * the error messages and then exit your program (stop running). 
     */
    public void saveImageWithoutErrors(String imageFileName) {
    	try { 
    		this.save(imageFileName);
    	} catch (Exception ex) {
    ...
    /** Loads the specified image file and returns a BufferedImage object
     * which can be displayed later by using g.drawImage(...)  */
    ...
    /**
     * Loads and plays the specified sound file.
     * @param soundFileName - Use .AU file format to be safe 
     * (maybe some WAV files will work...) (Sorry, no MP3)
     * You can use the free Audacity software to convert MP3 files into .AU format.
     */
    public void playSoundFile(String soundFileName) {
        ...
        try {
			...
			System.err.println("ERROR loading sound file named: " + soundFileName);
			System.err.println("Full path: " + soundFile.getAbsolutePath());			
			e.printStackTrace();
    		System.err.println("Because of error, exiting program...");
			System.exit(1);
		}
    ...
    /**
     * Loads and plays the specified MIDI music file. 
     * (Sorry, again, no MP3 support -- but you can find 
     * free MIDI files online... just be sure to give credit
     * to the artists if you use them in your program.) 
     * 
     * @param midiFileName - e.g. AwesomeMusic.mid
     */
    public void playMIDIMusicFile(String midiFileName) {
		...
		try {
			...
			System.err.println("ERROR loading midi music file named: " + midiFileName);
			System.err.println("Full path: " + midiFile.getAbsolutePath());			
    ...
    public void setVisible(boolean visible) {
        ...
        if (isGraphical()) {
        	...
        	} else if (!frame.isVisible() && visible) {
        		synchronized (getClass()) {
                    instances++;
                }
        	}
